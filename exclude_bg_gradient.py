...
def make_bg_static_random(video_id, global_idx, error_list, random_bg):
...
#     frame_indices = [an.get('image_id') for an in annotation.get('annotations')]
    ...
    #         total_masks_list.append(total_masks)
            ...
            total_masks = np.uint8(total_masks)

            rand_bg_mask = total_masks==0
            rand_bg_mask = rand_bg_mask * random_bg
#             print(rand_bg_mask.shape)
#             print(non_object_indices[0])
#             masked_rnd_bg = np.copy(random_bg)
#             masked_rnd_bg[:,non_object_indices] = 0
#             masked_rnd_bg = np.uint8(masked_rnd_bg)

            masked_img = np.uint8(original_img * total_masks.reshape((1080,1920,1))) + np.uint8(np.transpose(rand_bg_mask, (1, 2, 0)))
#             masked_img = np.uint8(original_img * total_masks.reshape((1080,1920,1)))
            im = Image.fromarray(masked_img)
            im.save(f"/usr/stud/gueley/Git/SCALOR/images_heavily_blurred_bw/{str(global_idx).zfill(6)}.jpeg")
        except Exception as e:
            raise e
            error_list.append((f'/storage/user/brasoand/motsyn2/frames/{video_id}/rgb/{str(frame_id).zfill(4)}.jpg', e))
        global_idx += 1
...
#         total_masks_list.append(total_masks)
        ...
        rand_bg_mask = total_masks==0
        rand_bg_mask = rand_bg_mask * random_bg
#         print(non_object_indices.shape)
#         masked_rnd_bg = np.copy(random_bg)
#         masked_rnd_bg[:, non_object_indices] = 0
#         masked_rnd_bg = np.uint8(masked_rnd_bg)
...
# def exclude_bbg(video_id, global_idx, error_list):
#     with open(f"/storage/user/brasoand/motsyn2/annotations/{video_id}.json") as fp:
#         annotation = json.load(fp)
#     print("Annotation File loaded.")
#     frame_src = f"/storage/user/brasoand/motsyn2/frames/{video_id}/rgb/"
...
#             masked_img = np.uint8(original_img * total_masks.reshape((1080,1920,1)))
#             im = Image.fromarray(masked_img)
#             im.save(f"/usr/stud/gueley/Git/SCALOR/static_bc_frames_2/{str(global_idx).zfill(6)}.jpeg")
#         except Exception as e:
#             error_list.append((f'/storage/user/brasoand/motsyn2/frames/{video_id}/rgb/{str(frame_id).zfill(4)}.jpg', e))
#         global_idx += 1
...
im = Image.open("heavily_blurred_birdview_img.jpeg")
random_bg = np.array(im)
random_bg = np.transpose(random_bg, (2, 0, 1))
...
for video_id in top_down_list:
    global_idx, error_list = make_bg_static_random(str(video_id).zfill(3), global_idx, error_list, random_bg)
    print(f"********************{global_idx}/{total_frames} done.***********************", flush=True)
