...
class SCALOR(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.constant_bg_what = torch.randn(10).repeat(args.batch_size, 1)
    ...
    def forward(self, seq, eps=1e-15):
        ...
        for i in range(seq_len):
            ...
            if lengths.max() != 0:
                ...
                # alpha_map_prop_list.append(alpha_map_prop.flatten(0,2))
            else:
            ...
            # alpha_map_disc_list.append(alpha_map_disc.flatten(0,2))
...
#                 print(f"self.constant_bg_what shape: {self.constant_bg_what.shape}", flush=True)
...
#                 bg = self.bg_dec(self.constant_bg_what)
#                 z_bg = self.constant_bg_what
...
#                 CHANGED
        ...
        # print(f"Alpha Map Shape: {alpha_map.shape}")
        ...
        #        kl_z_bg.flatten(start_dim=1).mean(dim=1), \
        #        log_imp_all.flatten(start_dim=1).sum(dim=1), \
        #        counting, log_disc_list, log_prop_list, scalor_log_list, alpha_map_disc_list, alpha_map_prop_list

class SCALORV2(nn.Module):

    def __init__(self, args):
        super(SCALORV2, self).__init__()
        self.args = args
        self.bg_what_std_bias = 0
        # self.constant_bg_what = torch.randn(10).repeat(args.batch_size, 1)
        # self.constang_bg_what_dict = {str(k):torch.randn(10).repeat(args.batch_size, 1) for k in range(video_id_list)}
    ...
    def forward(self, seq, video_embs, eps=1e-15):
